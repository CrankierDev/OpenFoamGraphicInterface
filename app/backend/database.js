...
boundaryField   :boundaryField
                ...
                db.run(OUTPUT_MODELS_INSERT, ["constant", "transportProperties", `// * * * * * * * * * FILE GENERATED BY OpenFOAMGUI * * * * * * * * * * * * * //
// * * * * * * * * * * * * UNIVERSIDAD DE CADIZ * * * * * * * * * * * * * * * //
    ...
    db.run(`CREATE TABLE simulations_info (id text, creationDate date, name text, simRoute text)`,
        (err) => {
            if(err) {
                ...
                db.run(SIMULATIONS_INFO_INSERT, ['default_sim', currentDate, 'Simulación por defecto', '/']);
            }
        }
    );
...
async function saveConstantData(simID, data) {
    ...
    try {
        db.run( CONSTANT_DATA_INSERT, [simID, data.transportModel, data.turbulenceModel,
...
async function saveSimulationInfo(simID, data) {
    ...
    try {
        ...
        db.run( SIMULATIONS_INFO_INSERT, [simID, currentDate, data.name, data.mesh]);
